...
public class Commands
{
    ...
    //20240216
    public ICommand OpenXtr { get; set; }
    public ICommand ToggleShowDebugState { get; set; }
    ...
    public Commands(Player player)
    {
        ...
        //20240216
        OpenXtr = new RelayCommand(OpenXtrAction);
        ToggleShowDebugState = new RelayCommand(ToggleShowDebugStateAction);
    }
    ...
    //20240216
    public void OpenXtrAction(object input)
    {
        if (input == null)
            return;

        string tName = input.ToString();
        Utils.Log("UpdateXtrQualityNameAndNotify -->" + tName);
        if (tName == "低频宽" || tName == "中频宽" || tName == "高频宽")
        {
            player.UpdateXtrQualityNameAndNotify(tName);
        }
        else
        {
            player.OpenAsync(input.ToString());
    ...
    public void ToggleShowDebugStateAction(object input)
    {
        if (input == null || !(input is bool))
            return;

        bool showDebug = (bool)input;
        Utils.Log("ToggleShowDebugStateAction -->" + showDebug);
        player.UpdateIsShowDebugAndNotify(showDebug);
    }
}
